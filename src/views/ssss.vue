<template>
      ...
      <el-tab-pane label="造价pdf处理" name="first">
        <div class="button-group">
          ...
          <el-select v-model="selectedConfig" placeholder="请选择配置" class="narrow-select">
            <!-- 动态渲染 el-option -->
            <el-option 
              v-for="(config, name) in apiConfigs" 
              :key="name" 
              :label="name" 
              :value="name"
            ></el-option>
  ...
  import aiAxios, { apiConfigs } from './pdfEditTable/config'; // 引入 apiConfigs
  ...
  const selectedConfig = ref('GLM-4-Flash');
  ...
  const submitPrompt = async () => {
    try {
      ...
      const fullContent = pdfStr.value ;
      ...
      for (let i = 0; i < fullContent.length; i += chunkSize) {
        ...
        const chunkResponse = await aiAxios(selectedConfig.value, chunk+","+ifpugFunctionPointEvaluationPrompt);
        text += chunkResponse;
      }
